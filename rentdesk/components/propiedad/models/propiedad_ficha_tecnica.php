...
$query = "SELECT 
    vp.nombre_1 || ' ' || vp.nombre_2 || ' | ' || vp.dni AS info_propietario,
    pc.nivel_propietario, 
    pc.id_propietario
FROM 
    propiedades.propiedad_copropietarios pc
JOIN 
    propiedades.vis_propietarios vp ON pc.id_propietario = vp.id
JOIN 
    propiedades.propietario_ctas_bancarias pcb ON pcb.id_propietario = pc.id_propietario
JOIN 
    propiedades.tp_banco tb ON tb.id = pcb.id_banco
JOIN 
    propiedades.tp_tipo_persona ttp ON vp.id_tipo_persona = ttp.id
WHERE 
    pc.id_propiedad = $ficha_tecnica 
    --AND pcb.id = pc.id_cta_bancaria
    AND pc.habilitado = true

UNION

SELECT pb.nombre ||' | ' || pb.rut as info_propietario, pc.nivel_propietario, pc.id_propietario
FROM propiedades.propiedad_copropietarios pc
LEFT JOIN propiedades.persona_beneficiario pb ON pc.id_beneficiario = pb.id
LEFT JOIN propiedades.vis_propietarios vp ON pc.id_propietario = vp.id
LEFT JOIN propiedades.tp_banco tb ON tb.id = pb.cta_id_banco
WHERE pc.id_propiedad =  $ficha_tecnica 
AND pc.habilitado = true

ORDER BY 
    id_propietario, nivel_propietario ASC
	";
...
$num_pagina = round($inicio / $cant_rows) + 1;
$data = array("consulta" => $query, "cantRegistros" => $cant_rows, "numPagina" => $num_pagina);
$resultado2 = $services->sendPostNoToken($url_services . '/util/objeto', $data, []);
//var_dump($resultado);
